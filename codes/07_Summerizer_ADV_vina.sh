...
log_directory="../DOCKING/ADV_vina"

# Create a new directory to store the output file
output_directory="../DOCKING/ADV_vina"
mkdir -p "$output_directory"

# Check if any .log files exist in the directory
if ! ls "$log_directory"/*.log &> /dev/null; then
  echo "\e[31mNo .log files found in ADV_vina. Docking failed.\e[0m"
  exit 1
fi

# Create a new file to store the output
output_file="$output_directory/Summarized_AutoDVina_vina.txt"
...
if ls "../INPUTS"/*flex_receptor.pdbqt &> /dev/null; then
  ...
  for log_file in "$log_directory"/*.log; do
    ...
    printf "%-22s\tAutoDock_Vina_v1.2.3\tvina\t\t\t\tGeneric_Algorithm\t\t%-10s\t%-20s\t%-30s\n" \
