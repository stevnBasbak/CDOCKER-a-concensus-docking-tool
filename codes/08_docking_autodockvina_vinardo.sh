...
receptor_dir=../INPUTS
output_dir=../DOCKING/ADV_vinardo
working_dir=..
...
if [ -f "$flex_receptor_file" ]; then
    ...
    for ligand in "$ligand_dir"/*_split*.pdbqt; do
        ...
        ${vina} --ligand "$ligand" --receptor "$receptor" --flex "$flex_receptor_file" --config "$vina_input" --scoring vinardo --exhaustiveness 10 --cpu "$cores" --out "$output_file" > "$log_file"
    ...
    for ligand in "$ligand_dir"/*_split*.pdbqt; do
        ...
        output_file="$output_dir/${ligand_basename}_docked.pdbqt"
        log_file="$output_dir/${ligand_basename}_docking.log"
        ${vina} --ligand "$ligand" --receptor "$receptor" --config "$vina_input" --scoring vinardo --exhaustiveness 10 --cpu "$cores" --out "$output_file" > "$log_file"
...
echo "AutoDock Vina (vinardo scoring) is completed"
